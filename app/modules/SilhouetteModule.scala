...
class SilhouetteModule @Inject() extends AbstractModule with ScalaModule  {
  ...
  */
  ...
  @Provides
  def provideLinkedInProvider(httpLayer: HTTPLayer, socialStateHandler: SocialStateHandler, configuration: Configuration): SolidLinkedInProvider = {
    new SolidLinkedInProvider(httpLayer, socialStateHandler, configuration.underlying.as[OAuth2Settings]("silhouette.linkedin"))
  }

  @Provides
  def provideGitHubProvider(httpLayer: HTTPLayer, socialStateHandler: SocialStateHandler, configuration: Configuration): SolidGitHubProvider = {
    new SolidGitHubProvider(httpLayer, socialStateHandler, configuration.underlying.as[OAuth2Settings]("silhouette.github"))
  }
